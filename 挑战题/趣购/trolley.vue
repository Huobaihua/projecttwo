...
module.exports = {
  ...
  computed: {
    ...
    goodsDetail() {
      return this.total.map(item => `${item.name} x ${item.count}`).join(" ");
    },
  },
  methods: {
    ...
    addToCart(event) {
      if (this.dragItem) {
        let existingItem = this.cart.find(item => item.name === this.dragItem.name);
        if (existingItem) {
          existingItem.count++;
        } else {
          this.cart.push({
            name: this.dragItem.name,
            price: this.dragItem.price,
            count: 1,
          });
        }
        this.dragItem = null;
      }
      this.cartCount = this.cart.reduce((total, item) => total + item.count, 0);
      this.cartDetail = this.goodsDetail;
